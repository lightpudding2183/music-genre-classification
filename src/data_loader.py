...
def save_mel_spectrograms(dataset_path, n_mels=N_MELS, n_fft=N_FFT, hop_length=HOP_LENGTH, num_segments=10):
    ...
    num_samples_per_segment = int(samples_per_segment) # 3s samples
    expected_num_mel_vectors = math.ceil(num_samples_per_segment / hop_length)

    # Duyệt qua các thư mục thể loại
    for i, (dirpath, dirnames, filenames) in enumerate(os.walk(dataset_path)):
        if dirpath is not dataset_path:
            genre_label = dirpath.split("/")[-1] # Mac/Linux use /
